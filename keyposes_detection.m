function [keyposes_frames,I_localized,I_trimmed] = keyposes_detection(I,window_size,threshold)
%Keypose detection
%localization of mutual information to emphasize changes
I_sliding_mean=abs(movmean(I,window_size));  %total window size, will take the mean of (window_size) frames
for i=1:size(I,2)
...
while i<size(I_trimmed,2)-1 %to go through the whole array
    while negative_peaks(i)~=0  %will look for a 0. Finding a 0 mean that the start of the peak have been reached
        i=i+1;
    ...
    i=stop(k);  %set i as the next starting point for the loop that will search for the next start
...
for i=1:size(start,2)
    ...
    temp_frame=start(1,i)+(find(I_trimmed(1,start(1,i):stop(1,i))==peaks_test(i))-1);  %use of the 'find' function to get the frame where the minimal occur
    keyposes_frames(i)=temp_frame(1,1);
end
